...
public class FiveBallRightAuto extends CommandGroup {
  ...
  public FiveBallRightAuto() {
    ...
    addSequential(new TimedTurn(0.7, 0.6)); // turn right
    // 4.5 seconds used

    // Must travel back ~8 feet then slow down
    addSequential(new TimedDrive(-0.75, 0.5)); // drive to near ball
    // 6.5 sec

    // Intake the balls
    addParallel(new TimedIntakeIn(4));
    addSequential(new TimedDrive(-0.5, 3.55)); // about correct distance
    // 9.5 sec
    ...
    addSequential(new TimedDrive(0.75, 1.85)); // drive back to line
    // 13 seconds used
    addParallel(new TimedShooterOut(5)); // Rev up shooter
    addSequential(new TimedTurn(-0.7, 0.3)); // turn left toward goal. Check angle
